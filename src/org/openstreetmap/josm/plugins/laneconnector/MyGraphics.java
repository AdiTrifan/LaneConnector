...
public class MyGraphics extends JComponent implements MouseListener,
MouseMotionListener {
...
//		setPreferredSize(new Dimension(700, 500));
	...
	void doClear() {
		...
		String filename = "detalii_MyGraph";
		String path = Main.pref.getPluginsDirectory().getPath();
		filename = path + "\\" + "LaneConnectorFiles" + "\\" + "Node_"
	...
	public static void makeDir() throws IOException {
		String path = Main.pref.getPluginsDirectory().getPath();
		File directory = new File(path + "\\" + "LaneConnectorFiles");
	...
	public void paint(Graphics g) {
		...
		//marimea initiala, setata de Oana
		g2d.fillRect(50, 0, 600, 500);
		g2d.fillRect(50,  0, 600, 600);
		

		
		//number of intersection streets
		...
		for (int k = 0; k < size; k++) {
			...
			System.out.println("way index" + LaneRelationsDialog.getJunctionWays().get(index));
...
//centre point 
			...
			for (int k = 0; k < size; k++) {
				...
				//parseaza din fiecare strada numarul de benzi(in debug in arata primele 2 campuri - nr of lanes lanes forw) 
				lanes.add(Integer.parseInt(LaneRelationsDialog
						.getJunctionWays().get(index).get("lanes")));
				System.out.println("integer parse int" +LaneRelationsDialog
						.getJunctionWays().get(index).get("lanes"));
			}
			...
			int upperReference = 800 - 15 * lanes.get(lanes.size() - 1);
			int lowerReference = 950 + 15 * lanes.get(lanes.size() - 1);
//			int leftReference = 228 - 15 * lanes.get(lanes.size() - 1);
			...
			for (int k = 0; k < size; k++) {
				...
				if (w.isFirstLastNode(LaneRelationsDialog.getSelectedNode())) {
					...
					if (firstn.equals(LaneRelationsDialog.getSelectedNode())) {
						...
						g2d.setColor(Color.BLACK); //aici nu stiu ce coloreaza
						//g2d.draw(path);
						...
						g2d.setColor(Color.BLACK); //nu stiu ce coloreaza
					...
					// way ends at junction
						...
						if (lastn.equals(LaneRelationsDialog.getSelectedNode())) {
							...
							g2d.setStroke(new BasicStroke(1));
							g2d.setColor(Color.BLACK);
							//se traseaza o linie intre linia care desparte sensurile de mers si punctul de intersectie
							//g2d.draw(path);
				...
				// way goes through junction
					...
					//in path se reprezinta legatura dintre punct si lini care desparte cele 2 benzi
					...
					g2d.setStroke(new BasicStroke(1));
					//aici se coloreaza ce este in partea stanga,, legatura dintre banda despartitoare de sens si punctul de intersectie
					g2d.setColor(Color.BLACK);  
					g2d.draw(path);
					...
					if (w.hasKey("lanes:forward")) {
						...
						for (int ind = 0; ind < Integer.parseInt(w
							...
							xprev = dx;
							yprev = dy;
//se coloreaza ce este in partea dreapta, linia disontinua, prima banda(te uiti de la stanga la dreapta)
						...
						g2d.setColor(Color.WHITE);
							...
							g2d.setStroke(new BasicStroke(1));
							//se coloreaza in partea dreapta punctele (Doar cele din partea stanga a mersului)
					g2d.setColor(Color.WHITE);
//							g2d.setColor(Color.blue);
						...
						g2d.setStroke(new BasicStroke(2));
	//se coloreaza in partea dreapta, linia de inceput de banda
						g2d.setColor(Color.BLACK);
...
//se ia doar partea de jos,benzile din dreapta, liniile discontinue despartiroare de benzi
							g2d.setColor(Color.WHITE);
							g2d.setStroke(new BasicStroke(2));
							drawDashedLine(g2d, x1p, y1p, x2p, y2p);

							g2d.setStroke(new BasicStroke(1));
							//punctele coresp benzilor de jos, doar cele din partea dreapta(un sens de mers)
							g2d.setColor(Color.BLACK);
							g2d.fill(new Ellipse2D.Double((xprev + x2p) / 2,
									(yprev + y2p) / 2, 9, 9));
						...
						g2d.setStroke(new BasicStroke(2));
					//pentru benzile de jos, dreapta(sens de mers), se coloreaza linia de inceput de banda(cea din partea dreapta)
					g2d.setColor(Color.BLACK);
						plf.moveTo(x1p, y1p);
						plf.lineTo(x2p, y2p);
						g2d.draw(plf);
	...
	//nu stiu ce e aici
					...
					pa2.lineTo(dx, dy);
					g2d.setStroke(new BasicStroke(3));
	//pentru benzile din partea de sus, se coloreaza linia despartitoare de sens
					g2d.setColor(Color.BLACK);
					g2d.draw(pa2);
					...
					if (w.hasKey("lanes:forward")) {
						...
						for (int ind = 0; ind < Integer.parseInt(w
							...
							xprev = dx;
							yprev = dy;
//se coloreaza liniile discontinue, despartitoare pentru benzile de sus 
							g2d.setColor(Color.WHITE);
							...
							g2d.setStroke(new BasicStroke(1));
							g2d.setColor(Color.WHITE);
	//se coloreaza punctele pentru benzile stanga sus, banda din stanga						
					...
					if (w.hasKey("lanes:backward")) {
						...
						for (int ind = 0; ind < Integer.parseInt(w
							...
							xprev = xcp2;
							yprev = ycp2;
//se coloreaza benzile din sus, cele din dreapta, liniile despartitoare de benzi
							g2d.setColor(Color.WHITE);
							g2d.setStroke(new BasicStroke(2));
							drawDashedLine(g2d, x1p, y1p, x2p, y2p);
						...
						g2d.setStroke(new BasicStroke(2));
						//inceputul  liniei de banda, partea dreapta
						g2d.setColor(Color.BLACK);
...
//							lines[i].y2);
	//reprezentarea legaturii dintre punctul intersectie si linile despartitoare de sens, insa cele de sus
			...
			if (l.getEllipse().contains(endX, endY)) {
				...
				for (LaneInformation lif : laneInfos) {
					if (lif.getEllipse().contains(startX, startY)) {
						...
						rel.addMember(new RelationMember("via",LaneRelationsDialog.getSelectedNode()));
						relations.add(rel);
